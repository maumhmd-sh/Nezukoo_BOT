...
console.error('❎ Terjadi kesalahan tak terduga:', code)
  console.error(chalk.red(`❌ BOT> akan dimulai ulang...`));
  start('main.js');
//--
    if (code === 0) return;
    fs.watchFile(args[0], () => {
  ...
  console.log(chalk.yellow(`📃 BOTScript by Nezukoo`));
  ...
  const packageJsonPath = path.join(path.dirname(currentFilePath), './package.json');
    ...
    console.log(chalk.cyan(`Deskripsi: ${packageJsonObj.deBOTription}`));
  ...
  return new Promise((resolve, reject) => {
    fs.readdir(folderPath, (err, files) => {
      if (err) {
        ...
        resolve({ folders, files: fileBOTount });
      }
    });
  });
}
...
process.on('unhandledRejection', () => {
  console.error(chalk.red(`❌ Penolakan janji tidak ditangani. BOT akan dimulai ulang...`));
  start('main.js');
});
